...
Users are prompted to select a fluorescent channel for each of the compensation controls from a drop-down menu. The results are saved to a csv file called "Compensation-Channels.csv" for later use. This channel selection process can be bypassed by suppling the name of a channel match file to the `channel_match` argument.
...
- density denstributions automically update with changes and display median fluorescent intensities for unstained and single stain controls.
...
Modifying the spillover values will automatically result in the plot being updated. The correct spillover value is such that the median of the stained population is equal to that of the unstained population (i.e. red line and purple line should meet in the center of the stained population). Looks like the PE-Cy7 control is over compensated in the APC-Cy7 channel, let's modify the value and click **save** to correct this:
