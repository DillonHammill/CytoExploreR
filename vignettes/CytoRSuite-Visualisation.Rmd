...
- sample merging
- **display** to sample events to speed up plotting
- stack density histograms with **density_stack**
- 2D scatter plots with default blue-red colour scale
- 2D **contour lines** overlays

`cyto_plot` is used internally throughout CytoRSuite so any plotCyto arguments can be passed to functions which generate plots, including `gate_draw` and `gate_edit`

## 2. `cyto_plot` Demonstration

Here we aim to document some of the key features of `cyto_plot` for visualation of flow cytometry data. To accomplish this goal we will use the `Activation` dataset supplied with **CytoRSuiteData**.
...
### 2.3 `cyto_plot`: 1-Dimensional Density Distibutions
...
Stacking of samples is supported through the `density_stack` argument which controls the percentage overlap of the stacked distributions. The `density_alpha` argument controls the degree of transparency of the fill colour.

```{r, fig.width = 6, fig.height = 6, fig.align='center'}
cyto_plot(TCells, channels = "CD4", density_fill = c("dodgerblue", "red"), density_stack = 0.5, density_alpha = 0.5)
```
