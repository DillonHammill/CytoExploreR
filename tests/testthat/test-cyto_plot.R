...
test_that("cyto_plot flowSet method", {
  ...
  expect_error(cyto_plot(fs),
  "Supply channel/marker(s) to construct the plot.",
  fixed = TRUE
  )
  
  # 1D density distributions
  p <- function() cyto_plot(cyto_extract(gs,"CD8 T Cells")[1:32],
            group_by = c("Treatment","OVAConc"),
            channels = "CD44",
            axes_trans = trans,
            legend = TRUE)
  
  expect_doppelganger("cyto_plot-flowSet-1D_001", p)
  
  # 2D scatter plots
  p <- function() cyto_plot(cyto_extract(gs, "CD4 T Cells")[1:32],
            group_by = c("Treatment","OVAConc"),
            channels = c("CD44","CD69"),
            axes_trans = trans,
            contour_lines = 5)
  
  expect_doppelganger("cyto_plot-flowSet-2D_001", p)
