...
#'   contructed, either a 1-D density distribution for a single channel or a 2-D
...
#'   default.
#' @param axes_label_text_font numeric to control the font axes labels, set to 1 for
#'   plain font by default. See \code{\link[graphics:par]{font}} for
#'   alternatives.
#' @param axes_label_text_size numeric to control the text size of axes labels, set
#'   to 1.1 by default.
...
#'   plot label(s) on the plot.
#' @param label_box_alpha numeric to control backgropund fill transparency of
...
setMethod(cyto_plot,
  ...
  definition = function(x,
    ...
    if (missing(channels)) {
      stop("Please supply the name(s) of the channel(s) or marker(s) to construct the plot.")
    }
...
#'   applied to the flowFrame internally and should be applied to the flowFrame
#'   prior to plotting.
#' @param group_by a vector of pData variables to sort and merge samples into groups prior
...
setMethod(cyto_plot,
  ...
  definition = function(x,
    ...
    # Check channels
    channels <- cyto_channel_check(x = getData(gh, "root"), channels, plot = TRUE)
...
setMethod(cyto_plot,
  ...
  definition = function(x,
    ...
    if (group_by != FALSE) {
      ...
      fr.lst <- .cyto_merge(gs, parent = parent, group_by = group_by, display = NULL)
      ...
      if (!is.null(overlay)) {
        overlay <- .cyto_overlay_check(x = gs, overlay = overlay, display = NULL)
        overlay <- .cyto_overlay_merge(x = gs, overlay = overlay, group_by = group_by, display = NULL)
      }
      ...
      # Plot titles
      if (missing(title)) {
        title <- sapply(1:length(gs), function(gh) {
          if (parent == "root") {
            parent <- "All Events"
          }
      ...
      # Extract gates
      if (!is.null(alias)) {
        gate <- lapply(1:length(gs), function(x) {
