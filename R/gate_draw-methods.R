...
#'
#' \code{gate_draw} is a convenient wrapper for the gating functions shipped with
#' \code{cytoRSuite} to facilitate analysis of flow cytometry by gate drawing.
#' Using \code{gate_draw} users can specify the type of gate(s) to be constructed
#' through the \code{type} argument and \code{gate_draw} will automatically
#' handle plotting the data and make calls to the relevant gating function(s) to
#' construct the gates around populations of interest. \code{gate_draw} has
...
#' @param label logical indicating whether to include \code{\link{cyto_plot_label}}
#'   for the gated population(s), \code{TRUE} by default.
#' @param density_smooth smooothing factor passed to
...
#' 
#' @export
setMethod(gate_draw, signature = "flowFrame", definition = function(x,
                                                                    channels = NULL,
                                                                    alias = NULL,
                                                                    display = 1 / length(x),
                                                                    type = "polygon",
  ...
  channels <- cyto_channel_check(fr, channels = channels, plot = TRUE)

  # Make one call to drawPlot
  if (plot == TRUE) {
    if (getOption("CytoRSuite_interact") == FALSE) {
      if (length(channels) == 2) {
        cyto_plot(fr, channels = channels, display = display, popup = FALSE, legend = FALSE, ...)
      } else {
        cyto_plot(fr, channels = channels, popup = FALSE, legend = FALSE, ...)
      }
    } else {
      if (length(channels) == 2) {
        cyto_plot(fr, channels = channels, display = display, popup = TRUE, legend = FALSE, ...)
      } else {
        cyto_plot(fr, channels = channels, popup = TRUE, legend = FALSE, ...)
      }
    }
  }
  ...
  if (length(type) == 1 & type[1] == "quadrant") {
    gates <- gate_quadrant_draw(fr = fr, channels = channels, alias = alias, display = display, plot = FALSE, label = label, ...)
  } else if (length(type) == 1 & type[1] == "web") {
    gates <- gate_web_draw(fr = fr, channels = channels, alias = alias, display = display, plot = FALSE, label = label, ...)
  } else {
    gates <- mapply(function(type, alias) {
      if (type == "polygon") {
        gate_polygon_draw(fr = fr, channels = channels, alias = alias, plot = FALSE, label = label, ...)
      } else if (type == "rectangle") {
        gate_rectangle_draw(fr = fr, channels = channels, alias = alias, plot = FALSE, label = label, ...)
      } else if (type == "interval") {
        gate_interval_draw(fr = fr, channels = channels, alias = alias, plot = FALSE, axis = axis, label = label, ...)
      } else if (type == "threshold") {
        gate_threshold_draw(fr = fr, channels = channels, alias = alias, plot = FALSE, label = label, ...)
      } else if (type == "boundary") {
        gate_boundary_draw(fr = fr, channels = channels, alias = alias, plot = FALSE, label = label, ...)
      } else if (type == "ellipse") {
        gate_ellipse_draw(fr = fr, channels = channels, alias = alias, plot = FALSE, label = label, ...)
      }
    }, type, alias)
  }
...
#' 
#' @export
setMethod(gate_draw, signature = "flowSet", definition = function(x,
                                                                  select = NULL,
                                                                  channels = NULL,
                                                                  alias = NULL,
                                                                  display = 1 / length(x),
  ...
  if (plot == TRUE) {
    if (getOption("CytoRSuite_interact") == FALSE) {
      if (length(channels) == 2) {
        cyto_plot(fs, channels = channels, display = display, popup = FALSE, legend = FALSE, group_by = "all", ...)
      } else {
        cyto_plot(fs, channels = channels, popup = FALSE, legend = FALSE, group_by = "all", ...)
      }
    } else {
      if (length(channels) == 2) {
        cyto_plot(fs, channels = channels, display = display, popup = TRUE, legend = FALSE, group_by = "all", ...)
      } else {
        cyto_plot(fs, channels = channels, popup = TRUE, legend = FALSE, group_by = "all", ...)
      }
    }
  }
...
#'   gate for all samples. If group_by is suppied a different gate will be
...
#' 
#' @export
setMethod(gate_draw, signature = "GatingSet", definition = function(x,
                                                                    group_by = NULL,
                                                                    select = NULL,
                                                                    parent = "root",
                                                                    alias = NULL,
  ...
  fltrsLst <- lapply(grps, function(fs) {
    ...
    if (is.na(group_by)) {
      ...
      title <- paste(pd[pd$name %in% sampleNames(fs), "group_by"][1], "\n", pnt)
    }
    ...
    if (plot == TRUE) {
      if (getOption("CytoRSuite_interact") == FALSE) {
        if (length(channels) == 2) {
          cyto_plot(x = gs[sampleNames(fs)], parent = parent, channels = channels, display = display, popup = FALSE, legend = FALSE, group_by = "all", title = title, ...)
        } else {
          cyto_plot(x = gs[sampleNames(fs)], parent = parent, channels = channels, popup = FALSE, legend = FALSE, group_by = "all", title = title, ...)
        }
      } else {
        if (length(channels) == 2) {
          cyto_plot(x = gs[sampleNames(fs)], parent = parent, channels = channels, display = display, popup = TRUE, legend = FALSE, group_by = "all", title = title, ...)
        } else {
          cyto_plot(x = gs[sampleNames(fs)], parent = parent, channels = channels, popup = TRUE, legend = FALSE, group_by = "all", title = title, ...)
        }
      }
    }
  ...
  if (is.null(gatingTemplate)) {
    ...
    for (i in 1:length(alias)) {
      pops[[i]] <- suppressWarnings(add_pop(
        gs = x, alias = alias[i], parent = parent, pop = pop, dims = paste(channels, collapse = ","), gating_method = "gate_draw",
        gating_args = list(gate = gates[[i]]), group_by = group_by, collapseDataForGating = TRUE,
    ...
    message(paste("Supplied gatingTemplate does not exist in working directory - writing", paste(gatingTemplate), "."))
