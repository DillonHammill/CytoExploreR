...
#'   \code{"geo mean"} or \code{"CV"}. Only count and percent statistics
...
cyto_plot_label2 <- function(x,
  ...
  if(!.all_na(label_stat)){
    label_stat <- unlist(lapply(label_stat, function(z) {.cyto_stat_check(z)}))
  }
  ...
  # Add labels to plot
  invisible(mapply(function(label_text,
                            st,
                            label_text_x,
                            label_text_y,
                            label_text_font,
