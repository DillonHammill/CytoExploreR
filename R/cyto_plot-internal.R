...
#' @param axes_label_text_font numeric indicating the font to use for title, set to 1
#'   for plain font by default. See \code{\link[graphics:par]{?par}} font for
#'   details.
...
#' fs <- Activation
#' cyto_plot_1d(fs[[1]], channel = "FSC-A", overlay = fs[[2]], density_fill = c("red", "blue"))
#' }
#'
...
#'
#' @noRd
setMethod(.cyto_plot_1d, signature = "flowFrame", definition = function(x, 
                                                                       channel = NULL, 
                                                                       axes_trans = NULL,
                                                                       overlay = NULL, 
                                                                       gate = NA, 
  ...
  xtext <- .cyto_axes_text(x = fr, channels = channel, trans = axes_trans)[[1]]
  ...
  if (!is.null(overlay)) {
    ...
    if(!density_modal){
      message("Overlays must be normalised to mode - setting density_modal to TRUE.")
      density_modal <- TRUE
    }
  ...
  frs.dens <- .cyto_density(x = frs, channel = channel, adjust = density_smooth, modal = density_modal, density_stack = density_stack)
  ...
  # Y Axis Title
  if (is.na(ylab) & !density_modal) {
    ylab <- "Density"
  } else if (is.na(ylab) & density_modal) {
  ...
  # Number of gates
  if(class(gate) %in% c("rectangleGate","polygonGate","ellipsoidGate","filters")){
    gates <- length(gate)
  }else if(inherits(gate, "list")){
    if(all(as.vector(sapply(gate, "class")) %in% c("rectangleGate","polygonGate","ellipsoidGate","filters"))){
      gates <- length(gate)
    }else if(all(as.vector(sapply(gate, "class")) == "list")){
      gates <- length(gate[[1]])
    }
  ...
  # Split arguments
  args <- list(xlab, ylab,
               title, title_text_font, title_text_size, title_text_col,
  ...
  names(args) <- c("xlab", "ylab",
                   ...
                   "label", "label_text", "label_stat", "label_text_font", "label_text_size", "label_text_col", "label_box_x", "label_box_y", "label_box_alpha",
                   "border_line_type", "border_line_width", "border_line_col")
  args <- .arg_split(x = args, channels = channel, n = lyrs, plots = 1, layers = lyrs, gates = gates)
  ...
  if (is.null(xtext) & ytext == FALSE) {
    ...
    abline(h = ofst, col = args[["density_line_col"]], lwd = args[["density_line_width"]], lty = args[["density_line_type"]])
    
    if(!is.na(args[["title"]])){
      title(main = args[["title"]], cex.main = args[["title_text_size"]], col.main = args[["title_text_col"]], font.main = args[["title_text_font"]])
    }
      
    title(xlab = args[["xlab"]], font.lab = args[["axes_label_text_font"]], col.lab = args[["axes_label_text_col"]], cex.lab = args[["axes_label_text_size"]])
    title(ylab = args[["ylab"]], mgp = c(2, 0, 0), font.lab = args[["axes_label_text_font"]], col.lab = args[["axes_label_text_col"]], cex.lab = args[["axes_label_text_size"]])
    ...
    title(xlab = args[["xlab"]], font.lab = args[["axes_label_text_font"]], col.lab = args[["axes_label_text_col"]], cex.lab = args[["axes_label_text_size"]])
    title(ylab = args[["ylab"]], mgp = c(3, 0, 0), font.lab = args[["axes_label_text_font"]], col.lab = args[["axes_label_text_col"]], cex.lab = args[["axes_label_text_size"]])
  ...
  if (!is.null(overlay) & args[["legend"]] != FALSE) {
    ...
    if(args[["legend"]] == "line"){
      ...
      legend(x = legend.x, y = legend.y, legend = legend_text, col = rev(args[["legend_line_col"]]), lty = rev(args[["density_line_type"]]), lwd = rev(args[["density_line_width"]]), xpd = TRUE, bty = "n", x.intersp = 0.5, cex = args[["legend_text_size"]], text.col = rev(args[["legend_text_col"]]), text.font = rev(args[["legend_text_font"]]))
    ...
    # Fill legend  
      ...
      legend(x = legend.x, y = legend.y, legend = legend_text, fill = rev(args[["legend_box_fill"]]), xpd = TRUE, bty = "n", x.intersp = 0.5, cex = args[["legend_text_size"]], text.col = rev(args[["legend_text_col"]]), text.font = rev(args[["legend_text_font"]]))
  ...
  if (is.null(overlay)) {
    if (valid_gates) {
      gate <- cyto_plot_gate(gate, channels = channel, gate_line_col = args[["gate_line_col"]], gate_line_width = args[["gate_line_width"]], gate_line_type = args[["gate_line_type"]])
    }
...
#'
#' @noRd
setMethod(.cyto_plot_1d, signature = "flowSet", definition = function(x, 
                                                                     channel, 
                                                                     axes_trans = NULL, 
                                                                     group_by = FALSE, 
                                                                     overlay = NULL, 
  ...
  channel <- cyto_channel_check(x = fs, channels = channel, plot = TRUE)
  ...
  # Get X Axis Breaks and Labels from trans if supplied
  xtext <- .cyto_axes_text(x = as(fs, "flowFrame"), channels = channel, trans = axes_trans)[[1]]
  
  # Axes limits
  if (is.null(xlim)) {
    xlim <- suppressWarnings(.cyto_plot_limits(x = fs, channels = channel, overlay = overlay, limits = limits)[[1]])
  }
  ...
  if (group_by[1] != FALSE) {
    ...
    if (is.null(overlay)) {
      ...
      if(all(density_stack == 0)){
        ...
        # title
        if (missing(title) & !all(group_by == "all")) {
          title <- names(fr.lst)
        } else if (missing(title) & all(group_by == "all")) {
      ...
      # Stacking - one panel
        ...
        # Legend text
        if(missing(legend_text)){
          legend_text <- lapply(unique(sp), function(x) names(fr.lst[sp == x]))
        }  
    ...
    # Overlay - separate panels with overlay  
      ...
      # Check overlay
      overlay <- .cyto_overlay_check(x = fs, overlay = overlay)
      
      # list of flowFrame lists to overlay (1 per group)
      overlay <- .cyto_overlay_merge(x = fs, overlay = overlay, group_by = group_by)
      
      # Layout
      layout <- .cyto_plot_layout(x = fr.lst, layout = layout)
    ...
    # Overlay - separate panels with overlay  
      ...
      # Legend text
      if(missing(legend_text)){
        legend_text <- paste0("layer", 1:(length(overlay[[1]]) + 1))
      }
  ...
  if(is.null(overlay)){
    ...
    if(all(density_stack == 0)){
      ...
      names(args) <- c("xlab", "ylab",
                       ...
                       "label", "label_text", "label_stat", "label_text_font", "label_text_size", "label_text_col", "label_box_x", "label_box_y", "label_box_alpha",
                       "border_line_type", "border_line_width", "border_line_col")
      args <- .arg_split(x = args, channels = channel, n = length(fr.lst), plots = length(fr.lst), layers = 1, gates = gates)
   
      # Call to cyto_plot_1d
      cnt <- 0
      mapply(function(fr, gate, xlab, ylab, title, title_text_font, title_text_size,title_text_col, density_fill, density_fill_alpha, density_line_type, density_line_width, density_line_col, axes_text_font, axes_text_size, axes_text_col, axes_label_text_font, axes_label_text_size, 
                      axes_label_text_col, legend, legend_text, legend_text_font, legend_text_size, legend_text_col, legend_line_col, legend_box_fill, gate_line_type, gate_line_width, gate_line_col, label, label_text, label_stat, label_text_font, label_text_size,
                      label_text_col, label_box_x, label_box_y, label_box_alpha, border_line_type, border_line_width, border_line_col, ...) {
        
        cnt <<- cnt + 1
        
        .cyto_plot_1d(x = fr, channel = channel, axes_trans = axes_trans, overlay = NULL, gate = gate, density_stack = 0, xlim = xlim, xlab = xlab, ylab = ylab, title = title, title_text_font = title_text_font, title_text_size = title_text_size, title_text_col = title_text_col, density_fill = density_fill,
                    ...
                    axes_label_text_font = axes_label_text_font, axes_label_text_size = axes_label_text_size, axes_label_text_col = axes_label_text_col, legend = legend, legend_text = legend_text, legend_text_font = legend_text_font, legend_text_size = legend_text_size, legend_text_col = legend_text_col,
                    ...
                    label_text_font = label_text_font, label_text_size = label_text_size, label_text_col = label_text_col, label_box_x = label_box_x, label_box_y = label_box_y, label_box_alpha = label_box_alpha, border_line_type = border_line_type, border_line_width = border_line_width,
      ...
      }, fr.lst, gate, args[["xlab"]], args[["ylab"]], args[["title"]], args[["title_text_font"]], args[["title_text_size"]], args[["title_text_col"]], args[["density_fill"]], args[["density_fill_alpha"]], args[["density_line_type"]], args[["density_line_width"]], args[["density_line_col"]], 
      args[["axes_text_font"]], args[["axes_text_size"]], args[["axes_text_col"]], args[["axes_label_text_font"]], args[["axes_label_text_size"]], args[["axes_label_text_col"]], args[["legend"]], args[["legend_text"]], args[["legend_text_font"]], args[["legend_text_size"]], 
    ...
    # Stacking - One panel
      ...
      names(args) <- c("xlab", "ylab",
                       ...
                       "label", "label_text", "label_stat", "label_text_font", "label_text_size", "label_text_col", "label_box_x", "label_box_y", "label_box_alpha",
                       "border_line_type", "border_line_width", "border_line_col")
      args <- .arg_split(x = args, channels = channel, n = length(fr.lst), plots = ceiling(length(fr.lst)/density_layers), layers = density_layers, gates = gates)
      
      
      # Call to cyto_plot
      mapply(function(sp, gate, xlab, ylab, title, title_text_font, title_text_size,title_text_col, density_stack, density_fill, density_fill_alpha, density_line_type, density_line_width, density_line_col, axes_text_font, axes_text_size, axes_text_col, axes_label_text_font, axes_label_text_size, 
                      axes_label_text_col, legend, legend_text, legend_text_font, legend_text_size, legend_text_col, legend_line_col, legend_box_fill, gate_line_type, gate_line_width, gate_line_col, label, label_text, label_stat, label_text_font, label_text_size,
                      label_text_col, label_box_x, label_box_y, label_box_alpha, border_line_type, border_line_width, border_line_col, ...) {
        
        .cyto_plot_1d(x = fr.lst[sp == sp][[1]], channel = channel, axes_trans = axes_trans, overlay = fr.lst[sp == sp][2:length(fr.lst[sp == sp])], gate = gate, density_stack = density_stack, xlim = xlim, xlab = xlab, ylab = ylab, title = title, title_text_font = title_text_font, title_text_size = title_text_size, title_text_col = title_text_col, density_fill = density_fill,
                     density_fill_alpha = density_fill_alpha, density_line_type = density_line_type, density_line_width = density_line_width, density_line_col = density_line_col, axes_text_font = axes_text_font, axes_text_size = axes_text_size, axes_text_col = axes_text_col,
                     axes_label_text_font = axes_label_text_font, axes_label_text_size = axes_label_text_size, axes_label_text_col = axes_label_text_col, legend = legend, legend_text = legend_text, legend_text_font = legend_text_font, legend_text_size = legend_text_size, legend_text_col = legend_text_col,
                     legend_line_col = legend_line_col, legend_box_fill = legend_box_fill, gate_line_type = gate_line_type, gate_line_width = gate_line_width, gate_line_col = gate_line_col, label = label, label_text = label_text, label_stat = label_stat, 
                     label_text_font = label_text_font, label_text_size = label_text_size, label_text_col = label_text_col, label_box_x = label_box_x, label_box_y = label_box_y, label_box_alpha = label_box_alpha, border_line_type = border_line_type, border_line_width = border_line_width,
      ...
      }, unique(sp), gate, args[["xlab"]], args[["ylab"]], args[["title"]], args[["title_text_font"]], args[["title_text_size"]], args[["title_text_col"]], args[["density_stack"]], args[["density_fill"]], args[["density_fill_alpha"]], args[["density_line_type"]], args[["density_line_width"]], args[["density_line_col"]], 
      args[["axes_text_font"]], args[["axes_text_size"]], args[["axes_text_col"]], args[["axes_label_text_font"]], args[["axes_label_text_size"]], args[["axes_label_text_col"]], args[["legend"]], args[["legend_text"]], args[["legend_text_font"]], args[["legend_text_size"]], 
      args[["legend_text_col"]], args[["legend_line_col"]], args[["legend_box_fill"]], args[["gate_line_type"]], args[["gate_line_width"]], args[["gate_line_col"]], args[["label"]], args[["label_text"]], args[["label_stat"]], args[["label_text_font"]],
      args[["label_text_size"]], args[["label_text_col"]], args[["label_box_x"]], args[["label_box_y"]], args[["label_box_alpha"]], args[["border_line_type"]], args[["border_line_width"]], args[["border_line_col"]], ...)
  ...
  # Overlay - separate panels  
    ...
    names(args) <- c("xlab","ylab",
                     ...
                     "label", "label_text", "label_stat", "label_text_font", "label_text_size", "label_text_col", "label_box_x", "label_box_y", "label_box_alpha",
                     "border_line_type", "border_line_width", "border_line_col")
    args <- .arg_split(x = args, channels = channel, n = (length(overlay[[1]]) + 1)*length(fr.lst), plots = length(fr.lst), layers = (length(overlay[[1]]) + 1), gates = gates)

    # Call to cyto_plot_1d
    cnt <- 0
    mapply(function(fr, gate, overlay, xlab, ylab, title, title_text_font, title_text_size,title_text_col, density_stack, density_fill, density_fill_alpha, density_line_type, density_line_width, density_line_col, axes_text_font, axes_text_size, axes_text_col, axes_label_text_font, axes_label_text_size, 
                    axes_label_text_col, legend, legend_text, legend_text_font, legend_text_size, legend_text_col, legend_line_col, legend_box_fill, gate_line_type, gate_line_width, gate_line_col, label, label_text, label_stat, label_text_font, label_text_size,
                    label_text_col, label_box_x, label_box_y, label_box_alpha, border_line_type, border_line_width, border_line_col, ...) {
      
      cnt <<- cnt + 1
      
      .cyto_plot_1d(x = fr, channel = channel, axes_trans = axes_trans, overlay = overlay, gate = gate, density_stack = density_stack, xlim = xlim, xlab = xlab, ylab = ylab, title = title, title_text_font = title_text_font, title_text_size = title_text_size, title_text_col = title_text_col, density_fill = density_fill,
                   density_fill_alpha = density_fill_alpha, density_line_type = density_line_type, density_line_width = density_line_width, density_line_col = density_line_col, axes_text_font = axes_text_font, axes_text_size = axes_text_size, axes_text_col = axes_text_col,
                   axes_label_text_font = axes_label_text_font, axes_label_text_size = axes_label_text_size, axes_label_text_col = axes_label_text_col, legend = legend, legend_text = legend_text, legend_text_font = legend_text_font, legend_text_size = legend_text_size, legend_text_col = legend_text_col,
                   legend_line_col = legend_line_col, legend_box_fill = legend_box_fill, gate_line_type = gate_line_type, gate_line_width = gate_line_width, gate_line_col = gate_line_col, label = label, label_text = label_text, label_stat = label_stat, 
                   label_text_font = label_text_font, label_text_size = label_text_size, label_text_col = label_text_col, label_box_x = label_box_x, label_box_y = label_box_y, label_box_alpha = label_box_alpha, border_line_type = border_line_type, border_line_width = border_line_width,
    ...
    }, fr.lst, gate, overlay, args[["xlab"]], args[["ylab"]], args[["title"]], args[["title_text_font"]], args[["title_text_size"]], args[["title_text_col"]], args[["density_stack"]], args[["density_fill"]], args[["density_fill_alpha"]], args[["density_line_type"]], args[["density_line_width"]], args[["density_line_col"]], 
    args[["axes_text_font"]], args[["axes_text_size"]], args[["axes_text_col"]], args[["axes_label_text_font"]], args[["axes_label_text_size"]], args[["axes_label_text_col"]], args[["legend"]], args[["legend_text"]], args[["legend_text_font"]], args[["legend_text_size"]], 
    args[["legend_text_col"]], args[["legend_line_col"]], args[["legend_box_fill"]], args[["gate_line_type"]], args[["gate_line_width"]], args[["gate_line_col"]], args[["label"]], args[["label_text"]], args[["label_stat"]], args[["label_text_font"]],
    args[["label_text_size"]], args[["label_text_col"]], args[["label_box_x"]], args[["label_box_y"]], args[["label_box_alpha"]], args[["border_line_type"]], args[["border_line_width"]], args[["border_line_col"]], ...)
...
#' @param axes_text_size character expansion for axis text.
#' @param axes_text_col colour of axis text.
#' @param axes_label_text_font numeric indicating the font to use for axes labels,
#'   set to 1 for plain font by default. See \code{\link[graphics:par]{?par}}
#'   font for details.
#' @param axes_label_text_size character expansion for axis labels.
#' @param axes_label_text_col colour of axis labels.
#' @param title_text_font numeric indicating the font to use for the title, set to 2
#'   for bold font by default. See \code{\link[graphics:par]{?par}} font for
#'   details.
#' @param title_text_size character expansion for plot title.
#' @param title_text_col colour for plot title.
...
#'   for a sold border.
#' @param border_line_width line width for plot border, set to 1 by default.
#' @param border_line_coline colour for plot border, set to "black" by
#'   default.l 
#' @param ... additional arguments passed to \code{\link[graphics:plot]{plot}}.
#'
#' @examples
...
#' library(CytoRSuiteData)
#' fs <- Activation
#' cyto_plot_2d(fs[[1]], channel = c("FSC-A", "SSC-A"), overlay = fs[[2]], point_col = c(NA, "purple"))
#' cyto_plot_2d(fs[[1]], channel = c("FSC-A", "SSC-A"), overlay = fs[[2]], point_col = c("black", "red"))
#' }
#'
#' @importFrom flowCore exprs parameters fsApply
#' @importFrom flowWorkspace pData
#' @importFrom MASS kde2d
#' @importFrom graphics plot axis title abline polygon contour legend points par box
...
#'
#' @noRd
setMethod(.cyto_plot_2d, signature = "flowFrame", definition = function(x, 
                                                                       channels, 
                                                                       axes_trans = NULL, 
                                                                       overlay = NULL, 
                                                                       contour_lines = 0, 
  ...
  # X axis limits
  if (is.null(xlim)) {
    xlim <- suppressWarnings(.cyto_plot_limits(x = fr, channels = channels, overlay = overlay, limits = limits)[[1]])
  }

  # Y axis limits
  if (is.null(ylim)) {
    ylim <- suppressWarnings(.cyto_plot_limits(x = fr, channels = channels, overlay = overlay, limits = limits)[[2]])
  }
  ...
  axs <- .cyto_axes_text(x = fr, channels = channels, trans = axes_trans)
  xtext <- axs[[1]]
  ytext <- axs[[2]]

  # overlay
  if (!is.null(overlay)) {
    overlay <- .cyto_overlay_check(x = fr, overlay = overlay, display = display)
  }
  ovn <- length(overlay)
  smp <- ovn + 1
  ...
  # Colours
  col_scale <- colorRampPalette(c("blue", "turquoise", "green", "yellow", "orange", "red", "darkred"))
  cols <- colorRampPalette(c("black", "darkorchid", "blueviolet", "magenta", "deeppink", "red4", "orange", "springgreen4"))

  # Point colours
  if(all(is.na(point_col))){
    
    point_col <- densCols(fr.exprs[, channels], colramp = col_scale)
    ...
    if(is.na(point_col)){
      point_col[is.na(point_col)] <- densCols(fr.exprs[, channels], colramp = col_scale)
    ...
    if(any(is.na(overlay_col))){
      ...
      lapply(match(NA, overlay_col), function(x){
        overlay_col[[x]] <<- densCols(exprs(overlay[[x]])[, channels], colramp = col_scale)
      })
    ...
    if(length(point_col) < smp){
      ...
      overlay_col <- c(point_col[-1], cols(ovn - length(point_col[-1])))
      point_col <- point_col[1]
  ...
  args <- list(xlab, ylab,
               ...
               contour_lines, contour_line_type, contour_line_width, contour_line_col,
               axes_text_font, axes_text_size, axes_text_col,
               axes_label_text_font, axes_label_text_size, axes_label_text_col,
               legend, legend_text, legend_text_font, legend_text_size, legend_text_col, legend_point_col,
               gate_line_type, gate_line_width, gate_line_col,
               label, label_text, label_stat, label_text_font, label_text_size, label_text_col, label_box_x, label_box_y, label_box_alpha,
               border_line_type, border_line_width, border_line_col)
  names(args) <- c("xlab","ylab",
                   ...
                   "label", "label_text", "label_stat", "label_text_font", "label_text_size", "label_text_col", "label_box_x", "label_box_y", "label_box_alpha",
                   "border_line_type", "border_line_width", "border_line_col")
  args <- .arg_split(x = args, channels = channels, n = smp, plots = 1, layers = smp, gates = gates)
  ...
  # Plot margins
  .cyto_plot_margins(x = fr, overlay = overlay, legend = args[["legend"]], legend_text = args[["legend_text"]], title = args[["title"]])

  # Plot
  if (nrow(fr) < 2) {
    graphics::plot(1, type = "n", axes = F, pch = args[["point_shape"]][1], cex.pts = args[["point_size"]][1], xlim = xlim, ylim = ylim, xlab = args[["xlab"]], ylab = args[["ylab"]], main = args[["title"]], cex.axis = args[["axes_text_size"]], col.axis = args[["axes_text_col"]], cex.lab = args[["axes_label_text_size"]], col.lab = args[["axes_label_text_col"]], font.main = args[["title_text_font"]], cex.main = args[["title_text_size"]], col.main = args[["title_text_col"]], bty = "n", ...)
    box(which = "plot", lty = args[["border_line_type"]], lwd = args[["border_line_width"]], col = args[["border_line_col"]])
  } else {
    if (is.null(xtext) & is.null(ytext)) {
      graphics::plot(fr.exprs, col = adjustcolor(point_col, args[["point_alpha"]][1]), pch = args[["point_shape"]][1], main = args[["title"]], xlab = args[["xlab"]], ylab = args[["ylab"]], xlim = xlim, ylim = ylim, cex = args[["point_size"]][1], cex.axis = args[["axes_text_size"]], col.axis = args[["axes_text_col"]], cex.lab = args[["axes_label_text_size"]], col.lab = args[["axes_label_text_col"]], font.main = args[["title_text_font"]], cex.main = args[["title_text_size"]], col.main = args[["title_text_col"]], bty = "n", ...)
      box(which = "plot", lty = args[["border_line_type"]], lwd = args[["border_line_width"]], col = args[["border_line_col"]])
    } else if (!is.null(xtext) & is.null(ytext)) {
      graphics::plot(fr.exprs, xaxt = "n", col = adjustcolor(point_col, args[["point_alpha"]][1]), pch = args[["point_shape"]][1], main = args[["title"]], xlab = args[["xlab"]], ylab = args[["ylab"]], xlim = xlim, ylim = ylim, cex = args[["point_size"]], cex.axis = args[["axes_text_size"]], col.axis = args[["axes_text_col"]], cex.lab = args[["axes_label_text_size"]], col.lab = args[["axes_label_text_col"]], font.main = args[["title_text_font"]], cex.main = args[["title_text_size"]], col.main = args[["title_text_col"]], bty = "n", ...)
      axis(1, at = xtext$at, labels = xtext$label, font.axis = args[["axes_text_font"]], col.axis = args[["axes_text_col"]], cex.axis = args[["axes_text_size"]])
      box(which = "plot", lty = args[["border_line_type"]], lwd = args[["border_line_width"]], col = args[["border_line_col"]])
    } else if (is.null(xtext) & !is.null(ytext)) {
      graphics::plot(fr.exprs, yaxt = "n", col = adjustcolor(point_col, args[["point_alpha"]][1]), pch = args[["point_shape"]][1], main = args[["title"]], xlab = args[["xlab"]], ylab = args[["ylab"]], xlim = xlim, ylim = ylim, cex = args[["point_size"]], cex.axis = args[["axes_text_size"]], col.axis = args[["axes_text_col"]], cex.lab = args[["axes_label_text_size"]], col.lab = args[["axes_label_text_col"]], font.main = args[["title_text_font"]], cex.main = args[["title_text_size"]], col.main = args[["title_text_col"]], bty = "n", ...)
      axis(2, at = ytext$at, labels = ytext$label, font.axis = args[["axes_text_font"]], col.axis = args[["axes_text_col"]], cex.axis = args[["axes_text_size"]])
      box(which = "plot", lty = args[["border_line_type"]], lwd = args[["border_line_width"]], col = args[["border_line_col"]])
    } else if (!is.null(xtext) & !is.null(ytext)) {
      graphics::plot(fr.exprs, xaxt = "n", yaxt = "n", col = adjustcolor(point_col, args[["point_alpha"]][1]), pch = args[["point_shape"]][1], main = args[["title"]], xlab = args[["xlab"]], ylab = args[["ylab"]], xlim = xlim, ylim = ylim, cex = args[["point_size"]], cex.axis = args[["axes_text_size"]], col.axis = args[["axes_text_col"]], cex.lab = args[["axes_label_text_size"]], col.lab = args[["axes_label_text_col"]], font.main = args[["title_text_font"]], cex.main = args[["title_text_size"]], col.main = args[["title_text_col"]], bty = "n", ...)
      axis(1, at = xtext$at, labels = xtext$label, font.axis = args[["axes_text_font"]], col.axis = args[["axes_text_col"]], cex.axis = args[["axes_text_size"]])
      axis(2, at = ytext$at, labels = ytext$label, font.axis = args[["axes_text_font"]], col.axis = args[["axes_text_col"]], cex.axis = args[["axes_text_size"]])
      box(which = "plot", lty = args[["border_line_type"]], lwd = args[["border_line_width"]], col = args[["border_line_col"]])
    }
  ...
  if (args[["contour_lines"]] != 0) {
    ...
    cyto_plot_contour(x = fr, channels = channels, contour_lines = args[["contour_lines"]], contour_line_type = args[["contour_line_type"]], contour_line_width = args[["contour_line_width"]], contour_line_col = args[["contour_line_col"]])
  ...
  if (!is.null(overlay)) {
    ...
    cyto_plot_overlay(x = overlay, channels = channels, point_shape = args[["point_shape"]][-1], point_size = args[["point_size"]][-1], point_col = overlay_col, point_alpha = args[["point_alpha"]][-1])
  ...
  if (!is.null(overlay) & args[["legend"]] == TRUE) {
    ...
    legend(x = legend.x, y = legend.y, legend = rev(args[["legend_text"]]), col = rev(legend_point_col), pch = rev(args[["point_shape"]]), pt.cex = rev(2*args[["point_size"]]), xpd = TRUE, bty = "n", x.intersp = 0.5)
  ...
  # Gates
  if (valid_gates) {
    gate <- cyto_plot_gate(gate, channels = channels, gate_line_col = args[["gate_line_col"]], gate_line_width = args[["gate_line_width"]], gate_line_type = args[["gate_line_type"]])
  }

  # Labels
  if (valid_gates & args[["label"]] == TRUE) {

    cyto_plot_label(x = fr, channels = channels, trans = axes_trans, text = args[["label_text"]], gates = gate, stat = args[["label_stat"]], text_x = args[["label_box_x"]], text_y = args[["label_box_y"]], text_size = args[["label_text_size"]], text_font = args[["label_text_font"]], text_col = args[["label_text_col"]], box_alpha = args[["label_box_alpha"]])
   
  # Labels without gates - 1 per layer 
  }else if(!valid_gates & !all(is.na(args[["label_text"]])) & args[["label"]]){
    ...
    mapply(function(label_text, label_stat, label_text_size, label_text_font, label_text_col, label_box_x, label_box_y, label_box_alpha){
      ...
      suppressMessages(cyto_plot_label(x = fr, channels = channels, gates = gate, trans = axes_trans, text = label_text, stat = label_stat, text_x = label_box_x, text_y = label_box_y, text_size = label_text_size, text_font = label_text_font, text_col = label_text_col, box_alpha = label_box_alpha))
    }, args[["label_text"]], args[["label_stat"]], args[["label_text_size"]], args[["label_text_font"]], args[["label_text_col"]], args[["label_box_x"]], args[["label_box_y"]], args[["label_box_alpha"]])
...
#' library(CytoRSuiteData)
#' fs <- Activation
#' cyto_plot_2d(fs, channel = c("FSC-A", "SSC-A"), overlay = fs[[2]], point_col = c(NA, "purple"))
#' cyto_plot_2d(fs, channel = c("FSC-A", "SSC-A"), overlay = fs[[2]], point_col = c("black", "red"))
#' }
#'
#' @importFrom flowCore exprs parameters fsApply
#' @importFrom flowWorkspace pData sampleNames
...
#'
#' @noRd
setMethod(.cyto_plot_2d, signature = "flowSet", definition = function(x, 
                                                                     channels, 
                                                                     axes_trans = NULL,
                                                                     overlay = NULL,
                                                                     contour_lines = 0,
  ...
  # Check channels
  channels <- cyto_channel_check(fs, channels = channels, plot = TRUE)
  ...
  # X axis limits
  if (is.null(xlim)) {
    xlim <- suppressWarnings(.cyto_plot_limits(x = fs, channels = channels, overlay = overlay, limits = limits)[[1]])
  }

  # Y axis limits
  if (is.null(ylim)) {
    ylim <- suppressWarnings(.cyto_plot_limits(x = fs, channels = channels, overlay = overlay, limits = limits)[[2]])
  }
  ...
  if (group_by[1] != FALSE) {
    ...
    # Return a list of merged flowFrames - keep sampling to cyto_plot flowFrame method
    fr.lst <- .cyto_merge(x = fs, group_by = group_by, display = NULL)
    
    # Title
    if (missing(title) & !group_by[1] == "all") {
      title <- names(fr.lst)
    } else if (missing(title) & group_by[1] == "all") {
      title <- "Combined Events"
    }

    # overlay - keep sampling for cyto_plot flowFrame method
    if (!is.null(overlay)) {
      overlay <- .cyto_overlay_check(x = fs, overlay = overlay, display = NULL)

      # list of flowFrame lists to overlay (1 per group)
      overlay <- .cyto_overlay_merge(x = fs, overlay = overlay, group_by = group_by, display = NULL)
    }
    ...
    # Convert fs to list of flowFrames - keep sampling for cyto_plot flowFrame method
    fr.lst <- lapply(seq(1, length(fs), 1), function(x) fs[[x]])
    names(fr.lst) <- fsnms
  ...
  args <- list(xlab, ylab,
               title, title_text_font, title_text_size, title_text_col,
               point_col, point_shape, point_size, point_alpha,
               contour_lines, contour_line_type, contour_line_width, contour_line_col,
               axes_text_font, axes_text_size, axes_text_col,
               axes_label_text_font, axes_label_text_size, axes_label_text_col,
               legend, legend_text, legend_text_font, legend_text_size, legend_text_col, legend_point_col,
  ...
  names(args) <- c("xlab","ylab",
                   ...
                   "label", "label_text", "label_stat", "label_text_font", "label_text_size", "label_text_col", "label_box_x", "label_box_y", "label_box_alpha",
                   "border_line_type", "border_line_width", "border_line_col")
  args <- .arg_split(x = args, channels = channels, n = (length(fr.lst) + ovn*(length(fr.lst))), plots = length(fr.lst), layers = ovn + 1, gates = gates)
  
  # Plots
  if (is.null(overlay)) {
      
    cnt <- 0
    mapply(function(fr, gate, xlab, ylab, title, title_text_font, title_text_size, title_text_col,
                    point_col, point_shape, point_size, point_alpha,
                    contour_lines, contour_line_type, contour_line_width, contour_line_col,
                    axes_text_font, axes_text_size, axes_text_col,
                    axes_label_text_font, axes_label_text_size, axes_label_text_col,
      ...
      .cyto_plot_2d(x = fr, channels = channels, display = display, axes_trans = axes_trans, gate = gate, xlim = xlim, ylim = ylim, xlab = xlab, ylab = ylab, title = title, title_text_font = title_text_font, title_text_size = title_text_size, title_text_col = title_text_col,
                    point_col = point_col, point_shape = point_shape, point_size = point_size, point_alpha = point_alpha, axes_text_font = axes_text_font, axes_text_size = axes_text_size, axes_text_col = axes_text_col, axes_label_text_font = axes_label_text_font, 
                   contour_lines = contour_lines, contour_line_type = contour_line_type, contour_line_width = contour_line_width, contour_line_col = contour_line_col, 
                   axes_label_text_size = axes_label_text_size, axes_label_text_col = axes_label_text_col, legend = legend, legend_text = legend_text, legend_text_font = legend_text_font, legend_text_size = legend_text_size, legend_text_col = legend_text_col, legend_point_col = legend_point_col,
                    gate_line_type = gate_line_type, gate_line_width = gate_line_width, gate_line_col = gate_line_col, label = label, label_text = label_text, label_stat = label_stat, label_text_font = label_text_font, label_text_size = label_text_size, label_text_col = label_text_col,
                    label_box_x = label_box_x, label_box_y = label_box_y, label_box_alpha = label_box_alpha, border_line_type = border_line_type, border_line_width = border_line_width, border_line_col = border_line_col, ...)
      ...
      }, fr.lst, gate, args[["xlab"]], args[["ylab"]], args[["title"]], args[["title_text_font"]], args[["title_text_size"]], args[["title_text_col"]],
      args[["point_col"]], args[["point_shape"]], args[["point_size"]], args[["point_alpha"]],
      args[["contour_lines"]], args[["contour_line_type"]], args[["contour_line_width"]], args[["contour_line_col"]],
      args[["axes_text_font"]], args[["axes_text_size"]], args[["axes_text_col"]],
      args[["axes_label_text_font"]], args[["axes_label_text_size"]], args[["axes_label_text_col"]],
      args[["legend"]], args[["legend_text"]], args[["legend_text_font"]], args[["legend_text_size"]], args[["legend_text_col"]], args[["legend_point_col"]],
      args[["gate_line_type"]], args[["gate_line_width"]], args[["gate_line_col"]],
      args[["label"]], args[["label_text"]], args[["label_stat"]], args[["label_text_font"]], args[["label_text_size"]], args[["label_text_col"]], args[["label_box_x"]], args[["label_box_y"]], args[["label_box_alpha"]],
      args[["border_line_type"]], args[["border_line_width"]], args[["border_line_col"]])
      
  } else if(!is.null(overlay)) {
      
    cnt <- 0
    mapply(function(fr, gate, overlay, xlab, ylab, title, title_text_font, title_text_size, title_text_col,
                    point_col, point_shape, point_size, point_alpha,
                    contour_lines, contour_line_type, contour_line_width, contour_line_col,
                    axes_text_font, axes_text_size, axes_text_col,
                    axes_label_text_font, axes_label_text_size, axes_label_text_col,
      ...
      .cyto_plot_2d(x = fr, channels = channels, display = display, axes_trans = axes_trans, overlay = overlay, gate = gate, xlim = xlim, ylim = ylim, xlab = xlab, ylab = ylab, title = title, title_text_font = title_text_font, title_text_size = title_text_size, title_text_col = title_text_col,
                   contour_lines = contour_lines, contour_line_type = contour_line_type, contour_line_width = contour_line_width, contour_line_col = contour_line_col, 
                   point_col = point_col, point_shape = point_shape, point_size = point_size, point_alpha = point_alpha, axes_text_font = axes_text_font, axes_text_size = axes_text_size, axes_text_col = axes_text_col, axes_label_text_font = axes_label_text_font, 
                    axes_label_text_size = axes_label_text_size, axes_label_text_col = axes_label_text_col, legend = legend, legend_text = legend_text, legend_text_font = legend_text_font, legend_text_size = legend_text_size, legend_text_col = legend_text_col, legend_point_col = legend_point_col,
                    gate_line_type = gate_line_type, gate_line_width = gate_line_width, gate_line_col = gate_line_col, label = label, label_text = label_text, label_stat = label_stat, label_text_font = label_text_font, label_text_size = label_text_size, label_text_col = label_text_col,
    ...
    }, fr.lst, gate, overlay, args[["xlab"]], args[["ylab"]], args[["title"]], args[["title_text_font"]], args[["title_text_size"]], args[["title_text_col"]],
    args[["point_col"]], args[["point_shape"]], args[["point_size"]], args[["point_alpha"]],
    args[["contour_lines"]], args[["contour_line_type"]], args[["contour_line_width"]], args[["contour_line_col"]],
    args[["axes_text_font"]], args[["axes_text_size"]], args[["axes_text_col"]],
    args[["axes_label_text_font"]], args[["axes_label_text_size"]], args[["axes_label_text_col"]],
...
#' 
#' @noRd
.arg_split <- function(x, channels, n, plots, layers, gates){
  ...
  args <- c("density_fill", "density_fill_alpha", "density_line_type", "density_line_width", "density_line_col",
            "legend_text", "legend_text_font", "legend_text_col", "legend_line_col", "legend_box_fill", "legend_point_col",
...
#' 
#' @noRd
.valid_gates <- function(x, channels){
  ...
  if(length(channels) == 1){
    ...
    if(inherits(x, "rectangleGate") | inherits(x, "filters")){
      ...
      if(all(as.vector(sapply(x, "class")) %in% c("rectangleGate","filters"))){
        valid_gates <- TRUE 
      }else if(all(as.vector(sapply(x, "class")) %in% c("list"))){
        
        if(all(as.vector(sapply(x, function(y){sapply(y,"class")})) %in% c("rectangleGate","filters"))){
          valid_gates <- TRUE
        }else{
          valid_gates <- FALSE
        }
    ...
    # Valid gates supplied?
    if(inherits(x, "rectangleGate") | inherits(x, "polygonGate") | inherits(x, "ellipsoidGate") | inherits(x, "filters")){
      valid_gates <- TRUE
    }else if(inherits(x, "list")){
      if(all(as.vector(sapply(x, "class")) %in% c("rectangleGate","polygonGate","ellipsoidGate","filters"))){
        valid_gates <- TRUE    
      }else if(all(as.vector(sapply(x, "class")) %in% c("list"))){
        
        if(all(as.vector(sapply(x, function(y){sapply(y,"class")})) %in% c("rectangleGate", "polygonGate", "ellipsoidGate","filters"))){
          valid_gates <- TRUE
        }else{
          valid_gates <- FALSE
        }
