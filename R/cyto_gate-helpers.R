...
cyto_gate_remove <- function(x,
  ...
  # REMOVE POPULATIONS FROM GH | GS
  if(length(gs_alias) > 0) {
    # DESCENDANTS
    gs_desc <- unname(
      cyto_nodes_kin(
        x,
    ...
    # ORDER NODES BASED ON GATINGSET
    gs_alias <- cyto_nodes_convert(
      x,
      nodes = gs_alias,
      path = "auto",
      sort = TRUE
    ...
    if(length(gs_alias) > 0) {
      message(
        paste0(
          "Removing the following gates from the ",
          cyto_class(x), ":\n",
          paste0(
            paste0(
              1:length(gs_alias),
  ...
  if(!is.null(gatingTemplate)) {
    ...
    # REMOVE FROM GATINGTEMPLATE
    if(length(gt_alias_rm) > 0 ) {
      # INFORM POPULATION REMOVAL
      message(
        paste0(
      ...
      # REMOVE NODES FROM GATINGTEMPLATE
...
cyto_gate_edit <- function(x,
  ...
  gates_gs_transposed <- structure(
    lapply(
      seq_along(gates_gs_transposed), 
      function(z){
        gates <- structure(
          lapply(
            seq_along(gates_gs_transposed[[z]]),
            function(y){
    ...
    function(z) {
      ...
      lapply(
        ...
        function(y) {
          ...
          # ANCHOR
          alias_anchor <- cyto_nodes_convert(
            x,
            nodes = alias[y],
            anchor = parent
          )
    ...
    if(nrow(gt_bool_chunk) > 0) {
      ...
      if(cyto_enquire(
        ...
        for(i in 1:nrow(gt_bool_chunk)) {
          # DESCENDANTS
          bool_desc <- gh_pop_get_descendants(
            x[[1]], 
