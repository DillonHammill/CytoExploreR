...
#' @param display numeric indicating the number of events to plot, set to 5000
#'   events by default.
...
#' }
#' @export
setMethod(spillover_edit, signature = "flowSet", definition = function(x,
                                                                       channel_match = NULL,
                                                                       spillover = NULL,
                                                                       display = 1,
                                                                       axes_trans = NULL, ...) {
  ...
  if (is.null(channel_match)) {
    ...
    if (inherits(channel_match, "data.frame") | inherits(channel_match, "matrix") | inherits(channel_match, "tibble")) {
      if (!all(c("name", "channel") %in% colnames(channel_match))) {
        stop("channel_match should contain columns 'name' and 'channel' to match each file name with a fluorescent channel.")
      }
      cm <- channel_match
    } else {
    ...
    if (inherits(spillover, "matrix") | inherits(spillover, "data.frame") | inherits(spillover, "tibble")) {
      ...
      if (getOption("CytoRSuite_wd_check") == TRUE) {
        if (.file_wd_check(spillover)) {
          ...
          stop("Supplied spillover file does not exist in the current working directory.")
        }
      } else {
        spill <- read.csv(spillover, header = TRUE, row.names = 1)
        spill <- as.matrix(spill)
  ...
  shinyApp(
    ...
    server <- function(input, output, session) {
      ...
      output$plot <- renderPlot({
        ...
        Nil.median <- median(exprs(.getRawData(fs.comp()[[input$Unstained]], axes_trans))[, input$ychannel])
        stain.median <- median(exprs(.getRawData(fs.comp()[[input$flowFrame]], axes_trans))[, input$ychannel])
        ...
        if (input$Uncomp == TRUE) {
          if (input$Comp == TRUE & input$NIL2 == FALSE) {
            ...
            cyto_plot_compensation(fs[[input$flowFrame2]], 
                                   channel = xchan, 
                                   axes_trans = axes_trans, 
                                   overlay = list(fs.comp()[[input$flowFrame2]], fs.comp()[[input$Unstained2]]), 
...
#' }
#' @export
setMethod(spillover_edit, signature = "GatingSet", definition = function(x,
                                                                         parent = NULL,
                                                                         channel_match = NULL,
                                                                         spillover = NULL,
                                                                         display = 1,
  ...
  spillover_edit(x = fs, channel_match = channel_match, spillover = spillover, axes_trans = axes_trans, display = display, ...)
})
