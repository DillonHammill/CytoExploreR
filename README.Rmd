...
Now that all the setup is complete, let's install all the necssary dependencies of **CytoExploreR**.

From within RStudio, run the following in the console to install the latest versions of [flowCore](https://github.com/RGLab/flowCore), [flowWorkspace](https://github.com/RGLab/flowWorkspace) and [openCyto](https://github.com/RGLab/openCyto) from Bioconductor.
```{r, eval = FALSE}
# Bioconductor
...
Now that all the dependencies are installed, let's move on to installing **CytoExploreR**.

To successfully install **CytoExploreR** users will first need to install **CytoExploreRData** which contains example datasets that will be used within **CytoExploreR** to demonstrate key features.
...
* customisable data transformations using `cyto_transform` which includes support for log, arcsinh, logicle and biexponential transformations
...
* exploratory visualisation of flowFrames, flowSets, GatingHierarchies and GatingSets using `cyto_plot`
...
For demonstration purposes, let's download the Compensation and Activation FCS files shipped with CytoExploreRData:
...
The basic steps to appropriate compensate for fluorescent spillover are outlined below. Refer to the Compensation vignette for a more detailed workflow.

+ Load Compensation-Samples into a GatingSet and prepare gatingTemplate
...
```{r, echo=FALSE, fig.align="center"}
    knitr::include_graphics('man/figures/README-Compensation-Gating.png')
```

+ Automated computation of spillover matrix using `cyto_spillover_compute`
...
+ Interactively edit computed spillover matrix using `cyto_spillover_edit`
...
```{r, echo=FALSE, fig.align="center"}
    knitr::include_graphics('man/figures/README-Compensation-Editing.png')
```

**4. Load and annotate Activation samples**

The basic steps to appropriately compensate, transform and manually gate the Activation samples are outlined below. Refer to the Manual Gating vignette for a more detailed workflow.
